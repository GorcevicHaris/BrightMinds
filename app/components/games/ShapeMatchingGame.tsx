...
export default function ShapeMatchingGame({ childId, level, onComplete, onClose, autoStart, isMonitor, monitorState }: GameProps) {
    ...
    const handleMoodAfterSelect = (mood: string) => {
        ...
        onComplete(score, duration, moodBefore, mood, { correct: correctCount, total: correctCount + incorrectCount });
    };
            ...
            <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center p-4 sm:p-10 overflow-hidden text-center">
                ...
                <div className="absolute inset-0 bg-slate-50">
                   <div 
                     ...
                     style={{ backgroundImage: "url('/images/pogodioblik.png')" }}
                   />
    ...
    // Intermediate 'Start Screen' removed as per optimization request
    ...
    return (
        <div className="bg-white/90 backdrop-blur-xl rounded-2xl md:rounded-[3rem] p-4 pt-6 md:p-10 shadow-2xl border border-white/50 w-full max-w-6xl mx-auto flex-1 flex flex-col animate-in fade-in duration-700 relative">
            ...
            {/* Compact Header Area */}
            <div className="flex justify-between items-center mb-8 md:mb-16 bg-white/60 backdrop-blur-2xl rounded-3xl md:rounded-[3rem] px-6 py-4 md:px-12 md:py-8 shadow-[0_20px_50px_rgba(0,0,0,0.05)] relative overflow-hidden border border-white">
                <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/5 rounded-full -mr-32 -mt-32 blur-3xl"></div>

                <div className="flex items-center gap-3 md:gap-6 relative z-10">
                    <div className="h-8 w-8 md:h-12 md:w-12 rounded-lg md:rounded-xl bg-indigo-600 shadow-lg flex items-center justify-center text-lg md:text-2xl ring-4 ring-indigo-50 border border-white">
                        ⭐
                    </div>
                    <div>
                        <span className="block text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Nivo</span>
                        <h3 className="text-base md:text-xl font-black text-slate-800 tracking-tight leading-none uppercase">{level}</h3>
                    </div>
                </div>

                <div className="flex items-center gap-3 md:gap-4 relative z-10">
                    <div className="bg-slate-900 rounded-lg md:rounded-xl px-3 py-1.5 md:px-6 md:py-3 text-center min-w-[70px] md:min-w-[100px] shadow-md">
                        <span className="block text-[7px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5 md:mb-1">Score</span>
                        <span className="text-lg md:text-2xl font-black text-white leading-none">{score}</span>
                    </div>

                    <div className={`rounded-lg md:rounded-xl px-3 py-1.5 md:px-6 md:py-3 text-center min-w-[60px] md:min-w-[90px] shadow-md border ${timeLeft <= 10 ? 'bg-rose-500 border-rose-200 animate-pulse' : 'bg-white border-indigo-50'}`}>
                        <span className={`block text-[7px] md:text-[9px] font-black uppercase tracking-widest mb-0.5 md:mb-1 ${timeLeft <= 10 ? 'text-rose-100' : 'text-slate-400'}`}>Time</span>
                        <span className={`text-lg md:text-2xl font-black leading-none ${timeLeft <= 10 ? 'text-white' : 'text-indigo-600'}`}>
                            {timeLeft}s
                        </span>
                    </div>
                </div>
            ...
            {/* Game Main Area */}
            <div className="flex-1 flex flex-col items-center justify-center gap-6 md:gap-12 pt-4 relative z-10 w-full">

                {/* Target prompt */}
                <div className="flex flex-col items-center gap-8 md:gap-12 animate-in fade-in zoom-in-95 duration-1000">
                    {targetShape && (
                        <div className="relative group cursor-default">
                            {/* Cinematic Glow Background */}
                            <div className="absolute inset-0 bg-white rounded-[3rem] md:rounded-[5rem] blur-[3rem] opacity-60 transition-all duration-700 group-hover:scale-125 group-hover:opacity-80"></div>
                            <div className="absolute -inset-4 bg-gradient-to-br from-indigo-500/20 via-white to-purple-500/20 rounded-[3.5rem] md:rounded-[5.5rem] blur-2xl opacity-40 group-hover:opacity-60 transition-opacity"></div>
                            
                            <div className="relative bg-white rounded-2xl md:rounded-3xl p-6 md:p-8 shadow-lg border border-slate-100 flex flex-col items-center justify-center ring-1 ring-slate-50 transition-all duration-300">
                                ...
                                <div className="absolute -top-3 px-6 py-1.5 bg-indigo-600 rounded-full text-[8px] md:text-[10px] font-black text-white uppercase tracking-widest shadow-md">
                                    Pronađi ovaj oblik
                                </div>
                            </div>
                        </div>
                ...
                {/* Multiple choice grid - Center content if few items */}
                        ...
                        {shapes.map((shape, idx) => (
                            ...
                            >
                                <div className="transform transition-all duration-500 group-hover:rotate-6">
                                    <ShapeSVG type={shape.type} color={shape.color} size={50} />
                                </div>

                                {feedback === "correct" && targetShape?.type === shape.type && (
                                    <div className="absolute -top-2 -right-2 bg-emerald-500 text-white p-1.5 md:p-2 rounded-full shadow-md animate-bounce border-2 border-white z-30">
