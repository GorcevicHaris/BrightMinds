...
export default function WelcomeAvatar({ childName, onLogoutConfirmed }: WelcomeAvatarProps) {
    ...
    useEffect(() => {
        ...
        setIsSmallMobile(mqSmall.matches);

        const handler = (e: MediaQueryListEvent) => setIsMobile(e.matches);
        const handlerSmall = (e: MediaQueryListEvent) => setIsSmallMobile(e.matches);

        mq.addEventListener('change', handler);
        mqSmall.addEventListener('change', handlerSmall);

        return () => {
            mq.removeEventListener('change', handler);
            mqSmall.removeEventListener('change', handlerSmall);
        };
    }, []);
        ...
        <>
            <div
                ...
                style={{
                    ...
                    transform: visible
                        ? `translateY(0) scale(${isSmallMobile ? 1.5 : isMobile ? 1.2 : 1})`
                        : `translateY(${isMobile ? '30px' : '-30px'}) scale(0.8)`,
                    opacity: visible ? 1 : 0,
